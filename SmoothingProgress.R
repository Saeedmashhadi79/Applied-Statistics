...
demand <- read.table('C:/Users/mvare/Desktop/POLIMI/POLIMI-S2/AS/Price_of_energy_2023-01-01_to_2023-12-31.txt', header = TRUE)
...
demand$Volume <- cumsum(demand$Quantita)
...
dates_formatted <- as.Date(demand$Data, format = "%d/%m/%Y")
# Convert back to character with "-" instead of "/"
demand$Data <- format(dates_formatted, "%d-%m-%Y")


smoothed_data <- smooth.spline(x =as.Date(demand$Data), y = demand$Volume)

# Spécifier l'origine pour la conversion en date
origin_date <- as.Date("01-01-1970")


# Créer un nouveau dataframe avec les données lissées
df_smoothed <- data.frame(Data = as.Date(smoothed_data$x, origin = origin_date), Volume= smoothed_data$y)
demand$Data <- as.Date(demand$Data)
df_smoothed$Data <- as.Date(df_smoothed$Data)

# Tracer le graphique avec les données originales et lissées
ggplot() +
  geom_line(data = demand, aes(x = Data, y = Volume), color = "blue", alpha = 0.5) +

  labs(x = "Date", y = "Volume", title = "Smoothed Vol over Time") +
  theme_minimal()
